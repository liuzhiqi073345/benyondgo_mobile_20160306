...
{
	$_SESSION['takegoods_id']=0;
	$_SESSION['takegoods_sn']='';
	header("Location:user.php?act=tg_login");
}
...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('wap_takegoods_ad',  get_wap_advlist('wap提货券幻灯广告', 4));  //wap首页幻灯广告位


    $sql = "select tg.*, tt.type_money_count from ".$ecs->table('takegoods')."AS tg left join ".$ecs->table('takegoods_type')."AS tt on tg.type_id = tt.type_id where tg.tg_id='$tg_id' ";
    $rowtg = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'take')
{
	...
	$takegoods_sn = $_SESSION['takegoods_sn'];
	$takegoods_id = isset($_POST['takegoods_id'])  ? intval($_POST['takegoods_id']) : 0;
    if (empty($takegoods_id))
	{
		show_message('错误操作', '返回上一页', 'takegoods.php', 'error');
	}
	else
	{ 
		$sql = "select goods_ids from ". $ecs->table('takegoods_goods') ." where tg_id='$takegoods_id' ";
		$goods_ids = $db->getOne($sql);
		if (empty($goods_ids))
		{
		...
		$sql = "select tg.used_time, tt.type_money_count from ". $ecs->table('takegoods') ."AS tg left join ".$ecs->table('takegoods_type')."AS tt on tg.type_id = tt.type_id where tg.tg_id='$takegoods_id' ";
	...
	$sql = "insert into ". $ecs->table('takegoods_order') . "(tg_id, tg_sn, user_id, goods_id, goods_name, country, province, city, district, address, consignee,mobile,email,add_time  )".
		" value('$takegoods_id', '$takegoods_sn', '$user_id', '$goods_id', '$goods_name', '$country', '$province','$city','$district', '$address', '$consignee' , '$mobile', '$email', '$add_time')";
	$db->query($sql);
	$tg_order_id = $db->insert_id();

	$add_time_sql = "select tg_order_id, used_time from ".$ecs->table('takegoods')." where tg_id='$takegoods_id'";
	...
	$sql = "update ". $ecs->table('takegoods') ." set tg_order_id='$tg_order_id', used_time='$add_time'  where  tg_id='$takegoods_id' ";
	$db->query($sql);
	...
	if ($_CFG['takegoods_send_email'] =='1' || $_CFG['takegoods_send_sms'] == '1' )
	{
		...
		{	
			...
			send_mail($_CFG['shop_name'], $admin_email, '卡号：'. $takegoods_sn .' 提货成功', $content1, 0);
			send_mail($_CFG['shop_name'], $email, '卡号：'. $takegoods_sn .' 提货成功', $content2, 0);
		}
		if ($_CFG['takegoods_send_sms'] == '1' )
		{
			$arrse = $_SESSION;
			include_once('../sms/sms.php');
			sendSMS($GLOBALS['_CFG']['sms_shop_mobile'], $content1);
