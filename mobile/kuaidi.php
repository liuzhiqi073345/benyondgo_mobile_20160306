...
{  
	...
	if (isset($_POST['yzcode']) && $_POST['yzcode']==$_SESSION['yzcode'])
	{
		...
		$send_name = $_POST['send_name'] ? trim($_POST['send_name']) : "";
		$send_tel = $_POST['send_tel'] ?   trim($_POST['send_tel']) : "";
		$send_region_id = $_POST['send_region_id'] ?   intval($_POST['send_region_id']) : "0";
		$send_address = $_POST['send_address'] ?   trim($_POST['send_address']) : "";
		$to_name = $_POST['to_name'] ? trim($_POST['to_name']) : "";
		$to_tel = $_POST['to_tel'] ?   trim($_POST['to_tel']) : "";
		$to_region_id = $_POST['to_region_id'] ?   intval($_POST['to_region_id']) : "0";
		$to_address = $_POST['to_address'] ?   trim($_POST['to_address']) : "";
		$goods_weight = $_POST['goods_weight'] ?  trim($_POST['goods_weight']) : "1";
		$goods_type = $_POST['goods_type'] ?  intval($_POST['goods_type']) : "1";
		$goods_name = $_POST['goods_name'] && $_POST['goods_name']!='填写物品名称' ?  trim($_POST['goods_name']) : "";
		...
		$money = $_POST['money'] ?  trim($_POST['money']) : "0";
		...
		foreach ($orderstatus_array AS $okey=>$oval)
		{
			...
			$sql="insert into ".$ecs->table('kuaidi_order_status').
						"(order_id, status_id, status_name, status_type, status_display) ".
						"values('$order_id', '$okey', '$oval[name]', '$oval[type]', '$status_display') ";
			$db->query($sql);
		}
