...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    else
    {
        ...
        /* 获得邮件服务器的参数设置 */
		/* 手机端未设置邮件服务器 则读取平台的商店设置 */
        $params['host'] = empty($GLOBALS['_CFG']['smtp_host'])?$GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('shop_config')." where code = 'smtp_host' and parent_id>0"):$GLOBALS['_CFG']['smtp_host'];
        $params['port'] = empty($GLOBALS['_CFG']['smtp_port'])?$GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('shop_config')." where code = 'smtp_port' and parent_id>0"):$GLOBALS['_CFG']['smtp_port'];
        $params['user'] = empty($GLOBALS['_CFG']['smtp_user'])?$GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('shop_config')." where code = 'smtp_user' and parent_id>0"):$GLOBALS['_CFG']['smtp_user'];
        $params['pass'] = empty($GLOBALS['_CFG']['smtp_pass'])?$GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('shop_config')." where code = 'smtp_pass' and parent_id>0"):$GLOBALS['_CFG']['smtp_pass'];
