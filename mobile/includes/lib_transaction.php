...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT headimg, user_name, password, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank," .
            " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, is_validated, validated, real_name, " .
			" card,face_card,back_card, country, province, city, district, address, status, is_surplus_open, surplus_password " .
...
function get_consignee_list($user_id)
{
	...
	$consignees = $GLOBALS['db']->getAll($sql);
	
	foreach ($consignees as $c_key => $consignee)
	{
		...
		$consignees[$c_key]['region'] = $re_infos[$consignee['province']]." ".$re_infos[$consignee['city']]." ".$re_infos[$consignee['district']];
	}
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$comment_s = 0;
		$shaidan_s = 0;
		$goods_list_r = get_order_goods1($row);
...
function get_order_goods1($order)
{
...
function get_order_detail($order_id, $user_id = 0)
{
	...
	$sql_regions = "SELECT region_id, region_name FROM ".$GLOBALS['ecs']->table('region')." WHERE region_id IN (".$order['province'].",".$order['city'].",".$order['district'].")";
	$regions = $GLOBALS['db']->getAll($sql_regions);
	$re_infos = array();
	foreach ($regions as $re_info)
	{
	...
	$order['region'] = $re_infos[$order['province']]." ".$re_infos[$order['city']]." ".$re_infos[$order['district']];
