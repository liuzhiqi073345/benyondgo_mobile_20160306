...
$sql_timezone = "SELECT value FROM " . $GLOBALS['ecs']->table('shop_config') .
                " WHERE code = 'timezone'";
$rs_timezone = $GLOBALS['db']->getOne($sql_timezone);
if (!empty($rs_timezone))
{
    if ($rs_timezone > 0)
    {
        $timezone = "Etc/GMT-" . $rs_timezone;
    }
    if ($rs_timezone < 0)
    {
        $timezone = "Etc/GMT+" . substr($rs_timezone, 1);
    }
    else
    {
        $timezone = "Etc/GMT";
...
if ($_CFG['shop_closed'] == 1)
{
    ...
    die('<div style="margin: 150px auto; text-align: center; font-size: 50px"><img src="themesmobile/68ecshopcom_mobile/images/icogantanhao.png"><p>' . $_LANG['shop_closed'] . '</p><p>' . $_CFG['close_comment'] . '</p></div>');
}
