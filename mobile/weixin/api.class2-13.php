...
class weixinapi{
	...
	**/
	...
	function getAwardNum($aid){
		...
		if($act['type'] == 1){
			$ymd = date('Y-m-d');
			$sql = "SELECT count(1) FROM " . $GLOBALS['ecs']->table('weixin_actlog') . " WHERE `uid` = '$uid' and createymd='$ymd'";
		}else{
			$sql = "SELECT count(1) FROM " . $GLOBALS['ecs']->table('weixin_actlog') . " WHERE `uid` = '$uid'";
		}
