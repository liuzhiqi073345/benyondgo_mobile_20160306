...
$(function(){
    ...
    <div class="touchweb-com_searchListBox <?php if ($this->_var['pager']['display'] == 'grid'): ?>openList<?php endif; ?>" id="goods_list"> <?php echo $this->fetch('library/goods_list.lbi'); ?> </div>
  </section>
...
?>
        <ul>
          <li class="filtrate_list_li"><a href="<?php echo $this->_var['cat']['url']; ?>" class="filtrate_list_li_a <?php if ($this->_var['current_cat_id'] == $this->_var['cat']['id']): ?>on<?php endif; ?>" style="padding-left:25px"><span><?php echo $this->_var['cat']['name']; ?></span></a>
            <ul>
...
?>
    ...
    <form method="post" action="category.php">
      ...
      <ul class="clearfix filtrate_address filtrate_list att_item" id="brands" style="display: block; -webkit-transform-origin: 0px 0px 0px; opacity: 1; -webkit-transform: scale(1, 1);">
        <?php $_from = $this->_var['brands']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'brand_0_87120900_1457169848');if (count($_from)):
    foreach ($_from AS $this->_var['brand_0_87120900_1457169848']):
?>
        <li <?php if ($this->_var['brand_0_87120900_1457169848']['selected']): ?>class="on"<?php endif; ?>  id="brand_<?php echo $this->_var['brand_0_87120900_1457169848']['brand_id']; ?>"><a href="javascript:get_brand('<?php echo $this->_var['brand_0_87120900_1457169848']['brand_id']; ?>')"><?php echo $this->_var['brand_0_87120900_1457169848']['brand_name']; ?></a></li>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      ...
      <div class="filtrate_price"onclick="return 1;"> 价格
        <input type="number" id="lp" value="<?php echo $this->_var['price_min']; ?>" name="price_min">
        ∼
        <input type="number" id="hp" value="<?php echo $this->_var['price_max']; ?>" name="price_max">
        元 </div>
      <?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'filter_attr_0_87153900_1457169848');$this->_foreach['filter_attr_list'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['filter_attr_list']['total'] > 0):
    foreach ($_from AS $this->_var['filter_attr_0_87153900_1457169848']):
        $this->_foreach['filter_attr_list']['iteration']++;
?>
      <div class="filtrate_category" > <a href="javascript:;" class="filtrate_category_a"><?php echo htmlspecialchars($this->_var['filter_attr_0_87153900_1457169848']['filter_attr_name']); ?><?php echo $this->_var['lang']['colon']; ?> <span class="up_down">全部</span></a> </div>
      <ul class="clearfix filtrate_address filtrate_list att_item" id="attrs_<?php echo $this->_foreach['filter_attr_list']['iteration']; ?>">
        <?php $_from = $this->_var['filter_attr_0_87153900_1457169848']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'attr');$this->_foreach['filter_attr'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['filter_attr']['total'] > 0):
    foreach ($_from AS $this->_var['attr']):
        $this->_foreach['filter_attr']['iteration']++;
?>
        <li <?php if ($this->_var['attr']['selected']): ?>class="on"<?php endif; ?> id="attr_<?php echo $this->_foreach['filter_attr_list']['iteration']; ?>_<?php if ($this->_var['attr']['attr_id']): ?><?php echo $this->_var['attr']['attr_id']; ?><?php else: ?>0<?php endif; ?>"> <a  href="javascript:get_attr_<?php echo $this->_foreach['filter_attr_list']['iteration']; ?>('<?php if ($this->_var['attr']['attr_id']): ?><?php echo $this->_var['attr']['attr_id']; ?><?php else: ?>0<?php endif; ?>')"><?php echo $this->_var['attr']['attr_value']; ?></a> <?php if ($this->_var['attr']['selected']): ?>
...
<?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'filter_attr_0_87267400_1457169848');$this->_foreach['filter_attr_list'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['filter_attr_list']['total'] > 0):
    foreach ($_from AS $this->_var['filter_attr_0_87267400_1457169848']):
        $this->_foreach['filter_attr_list']['iteration']++;
?>
