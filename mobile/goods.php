...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_info')
{
	$province = $_REQUEST['province'];
	$city = $_REQUEST['city'];
	$district = (empty($_REQUEST['district']) ? 0 : $_REQUEST['district']);
...
if(isset($_REQUEST['from']) && $_REQUEST['from'] == 'timeline')
{
	...
	// 这里goods.php的OAuth值暂定为4，请自行修改
	$url = "http://".$_SERVER['HTTP_HOST']."/weixin/oauth.php?id=0&oid=4&goods_id=".$goods_id;	 $url = $weixin->getOauthRedirect($url,1,'snsapi_base');
	 header("Location:$url");exit;
}

if(isset($_REQUEST['open_id']))
{
	$_SESSION['wx_openid'] =  $_REQUEST['open_id'];
}
