	...
	if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
	{
		...
		else
		{
			...
			if ( $num_total > intval($row_xg['buymax']) )
			{
				...
				$num_else = ($num_else < 0 ? 0 : $num_else);
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'id_card_num'     => empty($_POST['id_card_num'])  ? '' :   compile_str($_POST['id_card_num']),
        );
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	if($flow_type != CART_EXCHANGE_GOODS)
	{
		...
		$favourable_list = favourable_list($_SESSION['user_rank'],false,$_REQUEST['sel_goods']);
		if($favourable_list){
			$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
    ...
    $sql = "SELECT COUNT(*) " .
		   ...
		   " AND rec_id in (" . $_REQUEST['sel_goods'] . ")";
    ...
    else
	{
		$time_xg_now=gmtime();
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
		$sql="SELECT c.goods_number, g.goods_id, g.goods_name, g.is_buy, g.min_buynum, g.buymax, g.buymax_start_date, g.buymax_end_date, c.goods_attr_id " .
			 " FROM " . $ecs->table('cart') . " AS c " .
			 " LEFT JOIN " . $ecs->table('goods') . " AS g " .
			 " ON c.goods_id = g.goods_id " .
			 " WHERE " . $sql_where .
			 " AND c.rec_id in (" . $_REQUEST['sel_goods'] . ")";
	...
	$sql_regions = "SELECT region_id, region_name FROM ".$GLOBALS['ecs']->table('region')." WHERE region_id IN (".$consignee['province'].",".$consignee['city'].",".$consignee['district'].")";
	...
	$consignee['region'] = $re_infos[$consignee['province']]." ".$re_infos[$consignee['city']]." ".$re_infos[$consignee['district']];
	...
	// 输出默认自提点信息
	$other = (!empty($_SESSION['ziti_id'])? (" and id = " . $_SESSION['ziti_id']) : "");
	$sql = 'select * from ' . $ecs->table('pickup_point') .
			' where city_id=' . $_SESSION['flow_consignee']['city'] . $other;
	$pickup_point_list = $db->getRow($sql);
	$sql = 'select r.region_name, r.region_id from ' . $ecs->table('pickup_point') . ' p left join ' . $ecs->table('region') .
			' r on p.city_id=r.region_id where p.city_id=' . $_SESSION['flow_consignee']['city'];
...
{
	$sql = 'select * from ' . $ecs->table('pickup_point') .
			' where city_id=' . $_SESSION['flow_consignee']['city'];
	$pickup_point_list = $db->getAll($sql);

	foreach ($pickup_point_list as $p_key => $pickup_point)
	{
		$sql_regions = "SELECT region_id, region_name FROM ".$GLOBALS['ecs']->table('region')." WHERE region_id IN (".$pickup_point['province_id'].",".$pickup_point['city_id'].",".$pickup_point['district_id'].")";
		$regions = $GLOBALS['db']->getAll($sql_regions);
		$re_infos = array();
		foreach ($regions as $re_info)
		{
		...
		$pickup_point_list[$p_key]['region'] = $re_infos[$pickup_point['province_id']]." ".$re_infos[$pickup_point['city_id']]." ".$re_infos[$pickup_point['district_id']];
	}
...
elseif ($_REQUEST['step'] == 'zitic')
{
	$_SESSION['ziti_id'] = $_GET['id'];
	ecs_header("Location:./flow.php?step=checkout&p=zt\n");
}
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	/*  自提功能
		获取订单确认页选择的自提点
	*/
	if ($order['shipping_code'] == 'pups')
	{
		$pickup_point = isset($_POST['pickup_point']) ? $_POST['pickup_point'] : 0;
		$order['is_pickup'] = ($pickup_point > 0 ? 1 : 0);
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) ".
    ...
    if ($_CFG['send_service_email'] && $_CFG['service_email'] != '')
    {
        ...
        $smarty->assign('send_date', local_date($_CFG['time_format']));
    ...
    if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
    {
		...
		$content = "您有新订单,订单号:".$order['order_sn'].",收货人:".$order['consignee'].",电话:".$mobi."。【贵阳彼岸购保税店】";
		sendSMS($_CFG['sms_shop_mobile'],$content);
    }

	// 如果已经全额付款（余额、红包等），发短信
	if ($_CFG['sms_order_payed'] == '1' && $_CFG['sms_shop_mobile'] != '' && $order['order_amount'] == 0)
	{
		if ($order['mobile'] != '')
		{
			$mobi = $order['mobile'];
		...
		$content = "订单".$order['order_sn']."付款了。收货人:".$order['consignee'].",电话:".$mobi."。【贵阳彼岸购保税店】";
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
    if ($_SESSION['user_id'] > 0)
    {
        $rec_id = intval($_GET['id']);
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
        $goods_id = $db->getOne("SELECT  goods_id FROM " .$ecs->table('cart'). " WHERE rec_id = '$rec_id' AND " . $sql_where);
...
else
{
    ...
    $smarty->assign('real_goods_count', $cart_goods['total']['real_goods_count']);
 ...
 * @return  array
 */
function favourable_list($user_rank,$is_have=true,$sel_goods='')
{
    /* 购物车中已有的优惠活动及数量 */
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
	    ...
	    $favourable['gift'][$key]['goods_thumb'] = $GLOBALS['db']->getOne("select goods_thumb from " . $GLOBALS['ecs']->table('goods') . " where goods_id = " .$favourable['gift'][$key]['id']);
        ...
        /* 是否能享受 */
        $favourable['available'] = favourable_available($favourable,$sel_goods);
 ...
 * @return  bool
 */
function favourable_available($favourable,$sel_goods='')
{
    /* 会员等级是否符合 */
    ...
    /* 优惠范围内的商品总额 */
    $amount = cart_favourable_amount($favourable,$sel_goods);
 ...
 * @return  float
 */
function cart_favourable_amount($favourable,$sel_goods='')
{
    $sel_goods_where = (empty($sel_goods) ? '' : " AND c.rec_id IN (".$sel_goods.") ");
    $sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    /* 查询优惠范围内商品总额的sql */
