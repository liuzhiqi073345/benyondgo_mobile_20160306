...
switch ($act){
	...
	case "add"://add and edit
		...
		if($lid > 0){
			...
			if ($_GET['del'])
			{
				$sql = "DELETE FROM " . $ecs->table('weixin_actlist') . "  where lid=$lid";
			}
			else
			{
	...
	case "log":
		...
		file_put_contents('1.txt',var_export($qcode_list,true));
...
function qcode_list(){
	...
	$sql_count = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('weixin_actlog') . " left join " . $GLOBALS['ecs']->table('weixin_user') ." on " . $GLOBALS['ecs']->table('weixin_actlog') . ".uid=" . $GLOBALS['ecs']->table('weixin_user') . ".ecuid left join " . $GLOBALS['ecs']->table('weixin_act') . " on " . $GLOBALS['ecs']->table('weixin_actlog') . ".aid=" . $GLOBALS['ecs']->table('weixin_act') . ".aid where code!='' {$where}";
	...
	$sql = "SELECT " . $GLOBALS['ecs']->table('weixin_actlog') . ".*, " . $GLOBALS['ecs']->table('weixin_user') . ".nickname, " . $GLOBALS['ecs']->table('weixin_act') . ".title, ". $GLOBALS['ecs']->table('weixin_act') .".overymd FROM " . $GLOBALS['ecs']->table('weixin_actlog') . " left join " . $GLOBALS['ecs']->table('weixin_user') ." on " . $GLOBALS['ecs']->table('weixin_actlog') . ".uid=" . $GLOBALS['ecs']->table('weixin_user') . ".ecuid left join " . $GLOBALS['ecs']->table('weixin_act') . " on " . $GLOBALS['ecs']->table('weixin_actlog') . ".aid=" . $GLOBALS['ecs']->table('weixin_act') . ".aid where code!='' {$where} order by lid desc limit {$filter['start']},{$filter['page_size']}";
