...
$quick_ids = array(
	'CS_AWAIT_PAY'    => CS_AWAIT_PAY,
	'CS_AWAIT_SHIP'   => CS_AWAIT_SHIP,
	'SS_SHIPPED'      => SS_SHIPPED,
	'SS_SHIPPED_PART' => SS_SHIPPED_PART,
	'CS_FINISHED'     => CS_FINISHED,
	'OS_CANCELED'     => OS_CANCELED,
	'OS_BACKS'        => OS_BACKS,
	'PAY_COD'         => 'cod',
	'SHIP_PUPS'       => 'pups'
	);
$smarty->assign('quick_ids', $quick_ids);

$delivery_status = array(
	'DO_SHIPPED'      => DO_SHIPPED,
	'DO_SHIPPED_ING'  => DO_SHIPPED_ING,
	'DO_SHIPPED_PART' => DO_SHIPPED_PART,
	'DO_OK'           => DO_OK,
	'DO_BACK'         => DO_BACK,
	'DO_SHIPPED_BACK' => DO_SHIPPED_BACK,
	'DO_CONFIRMED'    => DO_CONFIRMED,
	'DO_PAYED'        => DO_PAYED,
	'DO_FINISHED'     => DO_FINISHED,
	'DO_INVALID'      => DO_INVALID,
	'DO_CANCELED'     => DO_CANCELED
	);
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('is_close',   $order_list['is_close']);
...
elseif($_REQUEST['act'] == 'get_status_list_child')
{
	require_once(ROOT_PATH . 'includes/cls_json.php');
	$json = new JSON;
	$result = array();
	$result['sel'] = !empty($_REQUEST['sel']) ? $_REQUEST['sel'] : '';
	$result['message'] = get_order_status_list($_REQUEST['parent']);
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (isset($_REQUEST['order_id']))
    {
		...
		// 获取订单其他信息
		$delivery_order = get_order_others($order_id);
		$smarty->assign('delivery_order', $delivery_order);
		
		// 格式化订单状态
		$payment = payment_info($order['pay_id']);
		$order['order_status_format'] = order_status_format($order['order_status'],$order['pay_status'],$order['shipping_status'], '', '', $payment['is_cod']);
		//追加订单状态对应的解释信息
		$status_info = array(
			'status_times'   => $delivery_order['status_times'],
			'pay_name'       => $order['pay_name'],
			'shipping_name'  => $order['shipping_name'],
			'delivery_infos' => $delivery_order['delivery_infos'],
			'invoice_no'     => $delivery_order['invoice_no']
			);
		
		$order['order_status_info'] = order_status_info($order['order_status_format']['status_format']['id'],$status_info);
		
		// 格式化地址信息
		$order['province_format'] = region_format($order['province']);
		$order['city_format']     = region_format($order['city']);
        //$order['id_card_num']  = region_format($order['id_card_num']);
		$order['district_format'] = region_format($order['district']);
		
		// 格式化包装、贺卡信息
		$order['pack_info'] = get_pack_info($order['pack_id']);
		$order['card_info'] = get_card_info($order['card_id']);
    ...
    if (empty($order))
    {
        $links[] = array('text' => '返回订单列表页', 'href' => 'order.php?act=list');
        sys_msg('此订单已不存在', 1, $links);
    }
	...
	$order['goods_list_prices'] = 0;
	$have_product = 0;
	$order['sended_all'] = 1;

	// 追加库存、已发货量，格式化空货品号
	foreach ($order['goods_list'] as $key => $goods_info)
	{
		if ($goods_info['product_id'] > 0)
		{
			$sql_numbers = "SELECT product_number FROM ".$GLOBALS['ecs']->table('products')." WHERE goods_id=".$goods_info['goods_id']." AND product_id=".$goods_info['product_id'];
			
			$have_product = 1;
		}
		else
		{
			$sql_numbers = "SELECT goods_number FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$goods_info['goods_id'];
		}
		$order['goods_list'][$key]['have_number'] = $GLOBALS['db']->getOne($sql_numbers);
		$order['goods_list'][$key]['sended'] = order_delivery_num($order['order_id'], $goods_info['goods_id'], $goods_info['product_id']);
		
		$goods_price = $goods_info['goods_price'];
		$goods_price = number_format($goods_price,2);
		$goods_number = $goods_info['goods_number'];

		$order['goods_list'][$key]['g_price'] = $goods_price;
		$order['goods_list'][$key]['g_prices'] = ($goods_price * $goods_number);
		$order['goods_list_prices'] += ($goods_price * $goods_number);
		$order['goods_list'][$key]['goods_number_last'] = ($goods_number - $order['goods_list'][$key]['sended']);
		if ($order['goods_list'][$key]['goods_number_last'] > 0)
		{
			$order['sended_all'] = 0;
		}

		// 格式化空货品号
		$order['goods_list'][$key]['product_id'] = ($order['goods_list'][$key]['product_id'] == 0 ? '' : $order['goods_list'][$key]['product_id']);

		if ($result['type'] == "edit_goods")
		{
			$order['goods_list'][$key]['goods_attr_list'] = get_goods_attrs($goods_info['goods_id'], $goods_info['goods_attr']);
			...
			if ($_REQUEST['search'])
			{
				$sql = "SELECT goods_id, goods_name FROM " . $GLOBALS['ecs']->table('goods') .
						...
						" AND is_on_sale = 1" .
						" AND is_alone_sale = 1" .
						" AND (goods_id LIKE '%" . mysql_like_quote($_REQUEST['search']) . "%'" .
						" OR goods_name LIKE '%" . mysql_like_quote($_REQUEST['search']) . "%'" .
						" OR goods_sn LIKE '%" . mysql_like_quote($_REQUEST['search']) . "%')" .
						" LIMIT 20";
				$res = $GLOBALS['db']->getAll($sql);
				
				if (!empty($res))
				{
					$order['goods_search'] = $res;
				}
				$order['is_search'] = 1;
			}
			else
	...
	$order['have_product'] = $have_product; // 声明是否含有带属性价格的商品。
	$order['goods_list_prices'] = price_format($order['goods_list_prices']);
    ...
    if (isset($_GET['print']))
    {
      ...
      /*  $lable_box['t_customer_idcardnum']='123';
$smarty->assign('showtt',$lable_box['t_customer_idcardnum']);*/
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        if ($shipping['print_model'] == 2)
        {
            ...
            $lable_box['t_customer_idcardnum'] = $order['id_card_num']; //收件人-身份证号码
    ...
    else
    {
        ...
        $smarty->assign('showtt',$lable_box['t_customer_idcardnum']);
        /* 显示模板 */       
        foreach (address_list($order['user_id']) as $key) {     
            foreach (address_list($order['user_id']) as $address)
            {
                if ($address['id_card_num']!= '')
                {
                    $getstr=$address['id_card_num'];   
                    $num=1;
                    break;
                }
            }
        ...
        $smarty->assign('numberget',$getstr);
        assign_query_info();
...
elseif ($_REQUEST['act'] == 'delivery_list')
{
	...
	/* 获得可选发货单状态 */
	$delivery_status_list = get_delivery_status_list();
	$smarty->assign('delivery_status_list', $delivery_status_list);
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    {
        $links[] = array('text' => '返回发货单列表页', 'href' => 'order.php?act=delivery_list');
        sys_msg('此发货单已不存在', 1, $links);
    }
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT dg.*, g.goods_thumb " .
	...
	// 是否是打印发货单流程
	if (!empty($_REQUEST['delivery_print']) && $_REQUEST['delivery_print'] == 1)
	{
		// 打印发货单，读取相应模版，追加赋值
        $smarty->assign('is_one',       1);
        $smarty->assign('shop_name',    $_CFG['shop_name']);
        $smarty->assign('shop_url',     $ecs->url());
        $smarty->assign('shop_address', $_CFG['shop_address']);
        $smarty->assign('service_phone',$_CFG['service_phone']);
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && ($order['mobile'] != '' || $order['tel'] != ''))
        {
			...
			$content = "您的订单".$order['order_sn']."已于".local_date("Y-m-d H:i:s",time())."发货。【贵阳彼岸购保税店】";
...
elseif ($_REQUEST['act'] == 'step_post')
{
	...
	{
		/* 操作成功 */
		$links[] = array('text' => $_LANG['back'] . $_LANG['order_detail'], 'href' => 'order.php?act=info&order_id=' . $order_id);
		$links[] = array('text' => $_LANG['back_list'], 'href' => 'order.php?act=list');
		sys_msg($_LANG['act_ok'], 0, $links);
	}
    }
    /* 保存配送信息 */
    elseif ('shipping' == $step)
    {
		...
		foreach ($order_info['invoices'] as $invoice_info)
		{
			$invoice_nos[] = $_REQUEST['invoice_info_input_'.$invoice_info['d_id']];
			if ($_REQUEST['invoice_info_input_'.$invoice_info['d_id']] != $invoice_info['invoice_no'])
			{
				$sql_update = "UPDATE " . $GLOBALS['ecs']->table('delivery_order') . " SET invoice_no = '" . $_REQUEST['invoice_info_input_'.$invoice_info['d_id']] . "' WHERE delivery_id = " . $invoice_info['d_id'];
				$GLOBALS['db']->query($sql_update);
			}
		}
		$invoice_no = implode('<br>', $invoice_nos);
		...
		{
			if ($_POST['pickup_point_id'] > 0)
			{
				$order['is_pickup'] = 1;
				$order['pickup_point'] = $_POST['pickup_point_id'];
			}
			else
			{
				sys_msg("请选择具体自提点", 0, array());
			}
    ...
    elseif ('money' == $step)
    {
        ...
        $order['discount']      = isset($_POST['discount']) && floatval($_POST['discount']) >= 0 ? round(floatval($_POST['discount']), 2) : $order['discount'];
        $order['tax']           = round(floatval($_POST['tax']), 2);
        $order['shipping_fee']  = isset($_POST['shipping_fee']) && floatval($_POST['shipping_fee']) >= 0 ? round(floatval($_POST['shipping_fee']), 2) : $order['shipping_fee'];
        $order['insure_fee']    = isset($_POST['insure_fee']) && floatval($_POST['insure_fee']) >= 0 ? round(floatval($_POST['insure_fee']), 2) : $order['insure_fee'];
        $order['pay_fee']       = floatval($_POST['pay_fee']) >= 0 ? round(floatval($_POST['pay_fee']), 2) : $order['pay_fee'];
        $order['pack_fee']      = isset($_POST['pack_fee']) && floatval($_POST['pack_fee']) >= 0 ? round(floatval($_POST['pack_fee']), 2) : $order['pack_fee'];
        $order['card_fee']      = isset($_POST['card_fee']) && floatval($_POST['card_fee']) >= 0 ? round(floatval($_POST['card_fee']), 2) : $order['card_fee'];

        $order['money_paid']    = $old_order['money_paid'];
        $order['surplus']       = $old_order['surplus'];
        //$order['integral']      = 0;
        $order['integral']      = intval($_POST['integral']) >= 0 ? intval($_POST['integral']) : $old_order['integral'];
        $order['integral_money']= $old_order['integral_money'];
        $order['bonus_id']      = $old_order['bonus_id'];
...
elseif ($_REQUEST['act'] == 'operate')
{
   ...
   $msg = '';
 	...
 	define('GMTIME_UTC', gmtime()); 
		...
		{
			$sql_have = "SELECT product_number FROM ".$GLOBALS['ecs']->table('products')." WHERE goods_id=".$value['goods_id']." AND product_id=".$value['product_id'];
		}
		else
		{
			$sql_have = "SELECT goods_number FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$value['goods_id'];
		}
					
		if ($send_number > $GLOBALS['db']->getOne($sql_have))
		{
			/* 操作失败 */
			$links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
			sys_msg(sprintf($_LANG['act_good_vacancy'], $value['goods_name']), 1, $links);
		...
		else if (!empty($_POST['number_'.$value['rec_id']]))
		{
			$send_number = $_POST['number_'.$value['rec_id']];
			$send_number_all = 1;
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    if ('confirm' == $operation)
    {
        ...
        if ($cfg == '1')
        {
	    $order['email'] = $db->getOne("select email from ".$ecs->table('users')."where user_id = ".$order['user_id']);
            $tpl = get_mail_template('order_confirm');
            $smarty->assign('order', $order);
            $smarty->assign('shop_name', $_CFG['shop_name']);
            $smarty->assign('send_date', local_date($_CFG['date_format']));
...
elseif($_REQUEST['act']=='excel')
{
	...
	foreach($order_list['orders'] as $key => $order_info)
	{
		$data .= "<table border='1'>";
		$data .= "<tr><td colspan='2'>订单号：".$order_info['order_sn']."</td><td>用户名：".$order_info['user_name']."</td><td colspan='2'>收货人：".$order_info['consignee']."</td><td colspan='2'>联系电话：".$order_info['mobile']."</td></tr>"; 
		$data .= "<tr><td colspan='5'>送货地址：".$order_info['consignee_province']." ".$order_info['consignee_city']." ".$order_info['consignee_district']." ".$order_info['address']."</td><td colspan='2'>下单时间：".local_date('y-m-d H:i', $order_info['add_time'])."</td></tr>";
		$data .= "<tr bgcolor='#999999'><th>序号</th><th>货号</th><th>商品名称</th><th>市场价</th><th>本店价</th><th>购买数量</th><th>小计</th></tr>";
		foreach($order_info['goods_list'] as $keys => $goods_info)
		{
			$keyss = $keys+1;
			$data .= "<tr><th>".$keyss."</th><th>".$goods_info['goods_sn']."</th><th>".$goods_info['goods_name']."</th><th>".$goods_info['market_price']."</th><th>".$goods_info['goods_price']."</th><th>".$goods_info['goods_number']."</th><th>".($goods_info['goods_price']*$goods_info['goods_number'])."</th></tr>";
		}
		$data .= "<tr><th colspan='5'></th><th>合计</th><th>".$order_info['total_fee']."</th></tr>";
		$data .= "</table>";
		$data .= "<br>";
	}
...
else if ($_REQUEST['act'] == 'get_div_info')
{
	require_once(ROOT_PATH . 'includes/cls_json.php');
	$json = new JSON;
	...
	// 变量初始化
	$result = array();
	$result['div_name'] = !empty($_REQUEST['div_name']) ? $_REQUEST['div_name'] : 'order_model';
	$result['type'] = !empty($_REQUEST['type']) ? $_REQUEST['type'] : '';
	$invoice_no = !empty($_REQUEST['invoice_no']) ? $_REQUEST['invoice_no'] : '';
	...
	if ($result['div_name'] == "order_model")
	{
		$order_id = intval($_REQUEST['id']);
		$order = order_info($order_id);
		...
		foreach ($order['goods_list'] as $key => $goods_info)
		{
			...
			$order['goods_list'][$key]['have_number'] = $GLOBALS['db']->getOne($sql_numbers);
			$order['goods_list'][$key]['sended'] = order_delivery_num($order_id, $goods_info['goods_id'], $goods_info['product_id']);
		...
		if ($result['type'] == "payment")
		{
			/* 获取可选支付方式 */
			$payments = get_payments();
			$smarty->assign('payment_list',   $payments);
		}
		...
		if ($result['type'] == "shipping")
		{
			...
			$shippings = available_shipping_list($region);
			foreach ($shippings AS $key => $val)
			{
				$shippings[$key]['insure_format'] = price_format($val['insure'], false);
				
				$shipping_cfg = unserialize_config($val['configure']);
				$shippings[$key]['free_money_format'] = price_format($shipping_cfg['free_money'], false);
				$weight_price = order_weight_price($order_id);
				$shippings[$key]['shipping_fee'] = shipping_fee($val['shipping_code'],$val['configure'], $weight_price['weight'], $total['goods_price'], $weight_price['number']);
				$shippings[$key]['shipping_fee_format'] = ($order['total_fee'] >= $shipping_cfg['free_money']) ? 0 : price_format($shippings[$key]['shipping_fee'], false);
				
				if ($val['shipping_code'] == 'pups')
				{
					$sql = "select * from " . $GLOBALS['ecs']->table('pickup_point') .	" where city_id='" . $order['city'] . "'";
					$pickup_point_list = $GLOBALS['db']->getAll($sql);
					if (!empty($pickup_point_list))
					{
						$shippings[$key]['pickup_points'] = $pickup_point_list;
					}
				}
			}
			$smarty->assign('shipping_list',   $shippings);
		}

		// 查询发票信息
		if ($result['type'] == "invoice")
		{
			$inv_type = !empty($order['inv_type']) ? $order['inv_type'] : 'normal_invoice';
			$inv_type = $_REQUEST['inv_type'] ? $_REQUEST['inv_type'] : $inv_type;
			$order['inv_type'] = $inv_type;

			$inv_payee_type = !empty($order['inv_payee_type']) ? $order['inv_payee_type'] : 'individual';
			$inv_payee_type = $_REQUEST['inv_payee_type'] ? $_REQUEST['inv_payee_type'] : $inv_payee_type;
			$order['inv_payee_type'] = $inv_payee_type;
		...
		$divs = array(
			...
			'type'       => $result['type'],
			'title'      => $_LANG['div_name_'.$result['type']],
			'order_id'   => $order_id,
			'invoice_no' => $invoice_no
			);
	}
	else if ($result['div_name'] == "attr_model")
	{
		$order_id  = intval($_REQUEST['id']);
		$goods_id  = intval($_REQUEST['goods_id']);
		$attr_list = get_goods_attrs($goods_id);
	...
	// 向AJAX回调函数传值
	$result['div_infos'] = array(
		'div_name' => $result['div_name']
		);

	make_json_result($smarty->fetch($result['div_name'].'.htm'), '', array('div_infos' => $result['div_infos']));

	echo $json->encode($result);
}

else if ($_REQUEST['act'] == 'change_goods')
{
	require_once(ROOT_PATH . 'includes/cls_json.php');
	$json = new JSON;
	...
	$sql_od = "SELECT * FROM " . $GLOBALS['ecs']->table('order_goods') .
		" WHERE rec_id = " . $rec_id;
	$order_goods_info = $GLOBALS['db']->getRow($sql_od);
	
	$order_id = $order_goods_info['order_id'];
	$goods_id = $order_goods_info['goods_id'];
	$g_type = !empty($_REQUEST['g_type']) ? $_REQUEST['g_type'] : '';
	
	$attrs = explode(',',$_REQUEST['attr']);
	foreach ($attrs as $key => $attr)
	{
		$ele = explode('<=->', $attr);
		$goods_ele[$ele[0]][] = $ele[1];
	}

	if ($g_type == 'attr')
	{
		// 获取订单中该商品所有可选属性ID
		$sql_attr_ids = "SELECT DISTINCT(ga.attr_id) FROM " .
			$GLOBALS['ecs']->table('goods_attr') .
			" AS ga " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('attribute') .
			" AS a " .
			" ON ga.attr_id = a.attr_id " .
			" WHERE a.attr_type > 0 AND ga.goods_id = " . $goods_id;
		$attr_ids = $GLOBALS['db']->getCol($sql_attr_ids);
		
		$goods_attrs = array();
		foreach ($attr_ids as $attr_id)
		{
			// 获取此属性的公用信息
			$sql_attr_infos = "SELECT * FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id = " . $attr_id;
			$attr_infos = $GLOBALS['db']->getRow($sql_attr_infos);
			
			$attr_post = $goods_ele[$goods_id.'_'.$attr_id];

			foreach ($attr_post as $_post)
			{
				$goods_attrs[] = $attr_infos['attr_name'].':'.$_post;
				$sql_goods_attr_id = "SELECT goods_attr_id FROM " . $GLOBALS['ecs']->table('goods_attr') .
					" WHERE goods_id = " . $goods_id .
					" AND attr_id = " . $attr_id .
					" AND attr_value = '" . $_post . "'";
				$goods_attr_ids[] = $GLOBALS['db']->getOne($sql_goods_attr_id);
			}
		}
		$goods_attr = implode(' ', $goods_attrs);
		$goods_aid = implode(',', $goods_attr_ids);
		
		$sql_have = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods') .
			" WHERE order_id = " . $order_id .
			" AND goods_id = " . $goods_id .
			" AND goods_attr_id = '" . $goods_aid . "'" .
			" AND goods_attr = '" . $goods_attr . "'";
		
		if ($GLOBALS['db']->getOne($sql_have) > 0)
		{
			$result['error'] = "该商品相同属性已存在此订单中，请重新选择";
		}
		else
		{
			if (!empty($goods_aid))
			{
				$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_attr = '" . implode('|', explode(',', $goods_aid)) . "' LIMIT 0, 1";
				$prod = $GLOBALS['db']->getRow($sql);
				
				if ($prod['product_number'] > 0)
				{
					$product_id = $prod['product_id'];
				}
				else
				{
					$result['error'] = "该商品此属性库存不足，请重新选择";
				}
			}
		}
		...
		{
			$sql = "UPDATE " . $GLOBALS['ecs']->table('order_goods') .
				" SET goods_attr = '" . $goods_attr . "', " .
				" goods_attr_id = '" . $goods_aid . "', " .
	...
	if ($g_type == 'number')
	{
		...
		if($product_id)
		{
			$sql = "SELECT product_number FROM " .
				$GLOBALS['ecs']->table('products') .
		...
		{
			$result['error'] = $_LANG['goods_num_err'];
		}
	}
	if ($g_type == 'price')
	{
...
else if ($_REQUEST['act'] == 'add_goods')
{
	...
	$goods_attrss = !empty($_REQUEST['goods_attrs']) ? $_REQUEST['goods_attrs'] : '';
	$goods_attrss = !empty($goods_attrss) ? explode(',', $goods_attrss) : '';
	
	$sql_attr_count = "SELECT COUNT(a.attr_id) FROM " .
		$GLOBALS['ecs']->table('goods_attr') .
		" AS ga " .
		" LEFT JOIN " .
		...
		" AND a.attr_type = 1 " .
		" GROUP BY a.attr_id";
	$attr_count = $GLOBALS['db']->getOne($sql_attr_count);

	if (empty($goods_attrss) && $attr_count > 0)
	{
		$result['error_attr'] = 1;
	...
	else
	{
		...
		if ($attr_count > 0)
		{
			foreach ($goods_attrss as $goods_attr_info)
			{
				$goods_attr_id1 = end(explode('_',reset(explode('<=->', $goods_attr_info))));
				$sql_attr = "SELECT attr_name FROM " .
					$GLOBALS['ecs']->table('attribute') .
					" WHERE attr_id = " . $goods_attr_id1;
				
				$goods_attrs[] = $GLOBALS['db']->getOne($sql_attr).":".end(explode('<=->', $goods_attr_info));
				
				$sql_gid = "SELECT goods_attr_id FROM " .
					$GLOBALS['ecs']->table('goods_attr') .
					" WHERE attr_value = '" . end(explode('<=->', $goods_attr_info)) . "' " .
					" AND attr_id = " . $goods_attr_id1 .
					" AND goods_id = " . $goods_id;
				$goods_attr_ids[] = $GLOBALS['db']->getOne($sql_gid);
			}
			
			if (!empty($goods_attr_ids) && !empty($goods_attrs))
			{
				$goods_attr_id = implode(',', $goods_attr_ids);
				$goods_attr = implode(' ', $goods_attrs);
			}
		}

		$sql_goods_info = "SELECT * FROM " . $ecs->table('goods') . " WHERE goods_id = " . $goods_id;
		$goods_info = $db->getRow($sql_goods_info);
		
		if (($goods_info['promote_price'] > 0) 
			&& ($goods_info['promote_start_date'] < gmtime()) 
			&& ($goods_info['promote_end_date'] > gmtime()))
		{
			$goods_price = $goods_info['promote_price'];
		}
		else
		{
		...
		$sql_order_have = "SELECT COUNT(*) FROM " .
			$GLOBALS['ecs']->table('order_goods') .
			" WHERE order_id = " . $order_id .
			" AND goods_id = " . $goods_id .
			" AND goods_attr_id = '" . $goods_attr_id ."'";
		$order_have = $GLOBALS['db']->getOne($sql_order_have);
		...
		if ($order_have > 0)
		{
			...
			$result['error'] .= "已存在此订单中，请重新选择";
		}
		else
		{
			if (!empty($goods_attr_id))
			{
				$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_attr = '" . implode('|', explode(',', $goods_attr_id)) . "' LIMIT 0, 1";
				$prod = $GLOBALS['db']->getRow($sql);
		...
		if ($result['error'] == "")
		{
			$sql = "INSERT INTO " .
				...
				" goods_sn, product_id, goods_number, market_price, " .
				" goods_price, is_real, " .
				" extension_code, parent_id, is_gift, " .
				" goods_attr, goods_attr_id) " .
				" VALUES " .
				" ('" . $order_id . "', '" . $goods_id . "', '" . $goods_info['goods_name'] . "', " .
				" '" . $goods_info['goods_sn'] . "', '" . $product_id . "', '1', '" . $goods_info['market_price'] . "', " .
				" '" . $goods_price . "', '" . $goods_info['is_real'] . "', " .
				" '" . $goods_info['extension_code'] . "', '0', '0', " .
				" '" . $goods_attr ."', '" . $goods_attr_id . "')";
			$db->query($sql);
...
else if ($_REQUEST['act'] == 'get_order_info_div')
{
	require_once(ROOT_PATH . 'includes/cls_json.php');
	$json = new JSON;
	
    $order_id = !empty($_REQUEST['order_id']) ? $_REQUEST['order_id'] : 0;
	$div_name = !empty($_REQUEST['div_name']) ? $_REQUEST['div_name'] : '';
    $smarty->assign('div_name', $div_name);
	$order = order_info($order_id);
	$order['goods_list'] = order_goods($order_id);
	$order['goods_list_prices'] = 0;
	$have_product = 0;
    ...
    make_json_result($smarty->fetch('order_info_model.htm'), '', null);
}

function get_goods_attrs($goods_id, $goods_attr_old = array())
{
	// 获取订单中该商品已选属性
	$goods_attr[] = explode(' ', trim($goods_attr_old));
    $attr = array();
    $arr  = array();
    foreach ($goods_attr AS $array_val)
    {
        foreach ($array_val AS $value)
        {
            $arr = explode(':', $value);//以 : 号将属性拆开
            $attr[] =  @array('name' => trim($arr[0]), 'value' => trim($arr[1]));
        }
    }
	...
	foreach ($attr_ids as $key => $attr_id)
	{
		...
		// 可选项格式转换
		$attr_values = explode("\n", $attr_infos['attr_values']);

		// 拼接此属性可选项下拉菜单1
		if ($attr_infos['attr_type'] == '1')
		{
			$html .= '<select name="'.$goods_id.'_'.$attr_id.'">';
		...
		foreach ($attr_values AS $opt)
		{
			...
			$sql_is_ga = "SELECT COUNT(goods_attr_id) FROM " .
				$GLOBALS['ecs']->table('goods_attr') .
				" WHERE attr_id = " . $attr_id .
				" AND goods_id = " . $goods_id .
				" AND attr_value = '" . $opt . "'";
			
			if ($GLOBALS['db']->getOne($sql_is_ga) > 0)
			{
				// 判断此属性是否已被选择
				$opt_olds = 0;
				foreach ($attr as $attr_v)
				{
					if ($attr_v['name'] == $attr_infos['attr_name'] && $attr_v['value'] == $opt)
					$opt_olds = 1;
				}
				...
				{
					$html .= ($opt_olds == 1) ?
						'<input name="'.$goods_id.'_'.$attr_id.'" type="checkbox" checked="checked" value="'.$opt.'"/>' . $opt . ' &nbsp; ' :
						'<input name="'.$goods_id.'_'.$attr_id.'" type="checkbox" value="'.$opt.'"/>' . $opt . ' &nbsp; ' ;
				}
			}
		}
		...
		$attrs[$key]['attr_info'] = $attr_infos; // 此属性的公用信息
		$attrs[$key]['select'] = $html;          // 此属性可选项
	}
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['status_list_child'] = empty($_REQUEST['status_list_child']) ? '' : trim($_REQUEST['status_list_child']);
        $filter['shipping'] = empty($_REQUEST['shipping']) ? '' : trim($_REQUEST['shipping']);
        $filter['payment'] = empty($_REQUEST['payment']) ? '' : trim($_REQUEST['payment']);
        $filter['user_name'] = empty($_REQUEST['user_name']) ? '' : trim($_REQUEST['user_name']);
        $filter['consignee'] = empty($_REQUEST['consignee']) ? '' : trim($_REQUEST['consignee']);
		
		$filter['quick_tab'] = empty($_REQUEST['quick_tab']) ? '' : intval($_REQUEST['quick_tab']);

        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'add_time' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
		$filter['is_close'] = empty($_REQUEST['is_close']) ? 0 : trim($_REQUEST['is_close']);
		
        $where = 'WHERE 1 ';
        if ($filter['goods_name'])
        {
			$sql_goods_ids = "SELECT goods_id FROM ".$GLOBALS['ecs']->table('goods').
				" WHERE goods_name LIKE '%" . mysql_like_quote($filter['goods_name']) . "%'";
			$goods_ids = $GLOBALS['db']->getCol($sql_goods_ids);

			$sql_order_ids = "SELECT DISTINCT(order_id) FROM ".$GLOBALS['ecs']->table('order_goods').
				" WHERE ".(empty($goods_ids) ? " 1=2 " : (count($goods_ids) > 1 ? "goods_id IN (".implode(',',$goods_ids).")" : "goods_id=".$goods_ids[0]));
			$order_ids = $GLOBALS['db']->getCol($sql_order_ids);

			if (!empty($order_ids))
			{
				$where .= " AND o.order_id ". (count($order_ids) > 1 ? "IN (".implode(',',$order_ids).")" : "=".$order_ids[0]);
			}
			else
			{
				$where .= " AND 1=2";
		...
		{
			$cod_ids = get_payments('cod');
			file_put_contents('1.txt',var_export($cod_ids,true));
			$cod_id  = $cod_ids[0]['pay_id'];

			switch ($filter['order_status'])
			{
				case CS_AWAIT_PAY :
					$where .= " AND o.order_status IN (".OS_CONFIRMED.",".OS_UNCONFIRMED.") AND IF(o.pay_id=".$cod_id.", o.pay_status=0 AND o.shipping_status>0, o.pay_status=0)";
					break;
				case CS_AWAIT_SHIP :
					$where .= " AND o.order_status IN (".OS_CONFIRMED.",".OS_UNCONFIRMED.") AND IF(o.pay_id=".$cod_id.", o.shipping_status=0, o.pay_status=2 AND o.shipping_status=0)";
					break;
				case SS_SHIPPED :
					if ($filter['status_list_child'])
					{
						switch ($filter['status_list_child'])
						{
							case SS_SHIPPED_PART :
								$where .= " AND o.order_status=".OS_SPLITING_PART." AND o.shipping_status=".SS_SHIPPED_PART;
								break;
							case SS_SHIPPED :
								$where .= " AND o.order_status=".OS_SPLITED." AND o.shipping_status=".SS_SHIPPED;
								break;
						}
					}
					else
					{
						$where .= " AND ((o.order_status=".OS_SPLITING_PART." AND o.shipping_status=".SS_SHIPPED_PART.") OR (o.order_status=".OS_SPLITED." AND o.shipping_status=".SS_SHIPPED."))";
					}
					break;
				case CS_FINISHED :
					$where .= " AND o.order_status=".OS_SPLITED." AND o.pay_status=".PS_PAYED." AND o.shipping_status=".SS_RECEIVED;
					break;
				case OS_CANCELED :
					$where .= " AND o.order_status IN (".OS_CANCELED.",".OS_INVALID.")";
					break;
				case OS_BACKS :
					$sql_back = "SELECT DISTINCT(order_id) FROM ".$GLOBALS['ecs']->table('back_order').
						" WHERE status_back NOT IN (".BACK_STATUS_NO.",".BACK_STATUS_SYS.",".BACK_STATUS_PER.")";
					$back_ids = $GLOBALS['db']->getCol($sql_back);
					$where .= " AND order_id IN (".implode(',',$back_ids).")";
					break;
			}
		}
		...
		if ($filter['payment'])
        {
            $where .= " AND pay.pay_code='".$filter['payment']."'";
        }

        if ($filter['shipping'])
        {
            $where .= " AND ship.shipping_code='".$filter['shipping']."'";
        }
        ...
        if ($filter['is_close'] == 1)
        {
            $where .= " AND o.order_status NOT IN (".OS_INVALID.",".OS_CANCELED.")";
        }
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o ".
				   ...
				   " LEFT JOIN ".$GLOBALS['ecs']->table('payment'). " AS pay ON pay.pay_id=o.pay_id ".
				   " LEFT JOIN ".$GLOBALS['ecs']->table('shipping'). " AS ship ON ship.shipping_id=o.shipping_id ".
				   $where;
        }
        else
        {
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.country, o.province, o.city, o.district, o.postscript, " .
					" o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.pay_id, " .
                    " o.pay_name, o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, " .
                    " o.extension_id, (" . order_amount_field('o.') . ") AS total_fee, " .
                    " IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer, o.is_pickup, " .
					...
					" pay.pay_code, ship.shipping_code " .
					" FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
					" LEFT JOIN " .$GLOBALS['ecs']->table('payment'). " AS pay ON pay.pay_id=o.pay_id " .
					" LEFT JOIN " .$GLOBALS['ecs']->table('shipping'). " AS ship ON ship.shipping_id=o.shipping_id " .
        ...
        foreach (array('goods_name', 'user_name', 'order_sn', 'shipping', 'payment') AS $val)
        {
	...
	$row[$key]['formatted_add_time'] = local_date('Y-m-d H:i',$value['add_time']);
		...
		/* 获取该订单包含商品信息 */
		$_goods_list = get_order_goods($row[$key]);
		$row[$key]['goods_list'] = $_goods_list['goods_list'];
		
		/* 格式化订单状态 */
		// 检查此单是否存在退款退货申请维修
		$sql_sh = "SELECT status_back, back_type FROM " . $GLOBALS['ecs']->table('back_order') .
			" WHERE order_id = " . $value['order_id'] . " ORDER BY back_id DESC LIMIT 1";
		$rs_sh = $GLOBALS['db']->getRow($sql_sh);
		if ($rs_sh['status_back'] >= 0 && 
			$rs_sh['status_back'] != BACK_STATUS_NO && 
			$rs_sh['status_back'] != BACK_STATUS_SYS && 
			$rs_sh['status_back'] != BACK_STATUS_PER && 
			$rs_sh['status_back'] != BACK_STATUS_OK)
		{
			$row[$key]['backing'] = $rs_sh['back_type'];
		}
		else
		{
			$row[$key]['backing'] = '';
		...
		/* 格式化地址信息 */
		$row[$key]['consignee_province'] = region_format($value['province']);
		$row[$key]['consignee_city'] = region_format($value['city']);
		$row[$key]['consignee_district'] = region_format($value['district']);
    }
    $arr = array(
		'orders' => $row, 
		'filter' => $filter, 
...
function get_order_goods($order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		// 检查此单此商品是否存在退款退货申请维修
		$sql_sh = "SELECT bg.status_back, bo.order_id, bo.back_type FROM " . $GLOBALS['ecs']->table('back_goods') . " AS bg " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('back_order') . " AS bo " .
			" ON bg.rec_id = bo.back_id " .
			" WHERE bg.goods_id = " . $row['goods_id'] .
			" AND bg.product_id = " . $row['product_id'] .
			" AND bo.order_id = " . $order['order_id'] . 
			" ORDER BY bg.back_id DESC LIMIT 1";
		$rs_sh = $GLOBALS['db']->getRow($sql_sh);
		...
		if ($rs_sh)
		{
			...
			switch ($rs_sh['status_back'])
			{
				...
				case BACK_STATUS_NO : 
					$row['shouhou'] .= '申请已被拒绝';
					break;
				case BACK_STATUS_SYS : 
					$row['shouhou'] .= '系统已自动取消';
					break;
				case BACK_STATUS_PER : 
					$row['shouhou'] .= '用户已自行取消';
					break;
				default : 
					$row['shouhou'] .= '正在进行';
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['consignee'] = empty($_REQUEST['consignee']) ? '' : trim($_REQUEST['consignee']);
        $filter['status'] = (!isset($_REQUEST['status']) || $_REQUEST['status'] == '') ? -1 : $_REQUEST['status'];
        $filter['start_time'] = empty($_REQUEST['start_time']) ? '' : trim($_REQUEST['start_time']);
        $filter['end_time'] = empty($_REQUEST['end_time']) ? '' : trim($_REQUEST['end_time']);
        ...
        if ($filter['status'] >= 0)
        {
			if ($filter['status'] == DO_BACK)
			{
				$back_sql = "SELECT order_id FROM ".$GLOBALS['ecs']->table('back_order').
					" WHERE back_type=".BACK_MONEY.
					" AND status_back NOT IN (".BACK_STATUS_NO.",".BACK_STATUS_SYS.",".BACK_STATUS_PER.")";
				$back_ids = $GLOBALS['db']->getCol($back_sql);
				$where_back = (empty($back_ids) ? "='-1'" : (count($back_ids) > 1 ? " IN (".implode(',',$back_ids).")" : "=".$back_ids[0]));
				$where .= " AND status = '" . DO_OK . "' AND order_id " . $where_back;
			}
			else if ($filter['status'] == DO_SHIPPED_BACK)
			{
				$back_sql = "SELECT order_id FROM ".$GLOBALS['ecs']->table('back_order').
					" WHERE back_type=".BACK_GOODS.
					" AND status_back NOT IN (".BACK_STATUS_NO.",".BACK_STATUS_SYS.",".BACK_STATUS_PER.")";
				$back_ids = $GLOBALS['db']->getCol($back_sql);
				$where_back = (empty($back_ids) ? "='-1'" : (count($back_ids) > 1 ? " IN (".implode(',',$back_ids).")" : "=".$back_ids[0]));
				$where .= " AND status = '" . DO_SHIPPED . "' AND order_id " . $where_back;
			}
			else
			{
				$back_sql = "SELECT order_id FROM ".$GLOBALS['ecs']->table('back_order').
					" WHERE status_back NOT IN (".BACK_STATUS_NO.",".BACK_STATUS_SYS.",".BACK_STATUS_PER.")";
				$back_ids = $GLOBALS['db']->getCol($back_sql);
				$where_back = (empty($back_ids) ? "" : " AND order_id ".( count($back_ids) > 1 ? " NOT IN (".implode(',',$back_ids).")" : "!=".$back_ids[0]));
				$where .= " AND status = '" . mysql_like_quote($filter['status']) . "' " . $where_back;
			}
        }
        ...
        /* 查询 */
        $sql = "SELECT delivery_id, invoice_no, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
    ...
    foreach ($row AS $key => $value)
    {
		...
		$sql_shipped_time= "SELECT * FROM ".$GLOBALS['ecs']->table('order_action').
			" WHERE order_id=".$value['order_id']." ORDER BY log_time DESC LIMIT 1";
		$rs = $GLOBALS['db']->getRow($sql_shipped_time);
		if ($rs && $rs['shipping_status'] == SS_SHIPPED)
		{
			$row[$key]['shipped_time'] = local_date($GLOBALS['_CFG']['time_format'], $rs['log_time']);
		}
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    if ($delivery)
    {
		...
		// 扩展发货单状态
		$sql_back = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('back_order')." WHERE order_id=".$delivery['order_id']." AND status_back NOT IN (".BACK_STATUS_NO.",".BACK_STATUS_SYS.",".BACK_STATUS_PER.")";
		switch ($delivery['status'])
		{
			case DO_OK : // 对于正常发货单
				$sql_back .= " AND back_type=".BACK_MONEY;
				$delivery['order_status'] = ($GLOBALS['db']->getOne($sql_back) > 0 ? DO_BACK : DO_OK);
				break;
			case DO_SHIPPED : // 对于已发货
				$sql_back .= " AND back_type=".BACK_GOODS;
				$delivery['order_status'] = ($GLOBALS['db']->getOne($sql_back) > 0 ? DO_SHIPPED_BACK : DO_SHIPPED);
				break;
		}
		// 格式化发货单状态
		$delivery['order_status_format'] = get_delivery_status_list($delivery['order_status']);
		
		// 格式化办事处信息
		$sql_agency = "SELECT * FROM ".$GLOBALS['ecs']->table('agency')." WHERE agency_id=".$delivery['agency_id'];
		$delivery['agency_info'] = ($GLOBALS['db']->getRow($sql_agency) ? $GLOBALS['db']->getRow($sql_agency) : '');

		// 获取订单其他信息
		$delivery_order = get_order_others($delivery['order_id'],$delivery);
		$delivery = array_merge($delivery,$delivery_order);
