...
elseif ($_REQUEST['act'] == 'top')
{
    // 获得管理员设置的菜单
    $lst = array();
    $nav = $db->GetOne('SELECT nav_list FROM ' . $ecs->table('admin_user') . " WHERE user_id = '" . $_SESSION['mobile_admin_id'] . "'");
    ...
    $smarty->assign('admin_id', $_SESSION['mobile_admin_id']);
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    "WHERE a.sender_id = b.user_id AND a.receiver_id = '$_SESSION[mobile_admin_id]' AND ".
...
{
    if (empty($_SESSION['mobile_last_check']))
    {
        $_SESSION['mobile_last_check'] = gmtime();
    ...
    /* 新付款的订单 */
    $sql = 'SELECT COUNT(*) FROM '.$ecs->table('order_info').
    ' WHERE pay_time >= ' . $_SESSION['mobile_last_check'];
...
elseif ($_REQUEST['act'] == 'save_todolist')
{
    ...
    $sql = "UPDATE" .$GLOBALS['ecs']->table('admin_user'). " SET todolist='" . $content . "' WHERE user_id = " . $_SESSION['mobile_admin_id'];
    $GLOBALS['db']->query($sql);
}

elseif ($_REQUEST['act'] == 'get_todolist')
{
    $sql     = "SELECT todolist FROM " .$GLOBALS['ecs']->table('admin_user'). " WHERE user_id = " . $_SESSION['mobile_admin_id'];
