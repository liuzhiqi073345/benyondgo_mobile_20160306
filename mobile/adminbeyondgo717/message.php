...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 获取管理员信息 */
        $result = $db->query('SELECT user_id FROM ' .$ecs->table('admin_user') . 'WHERE user_id !=' . $_SESSION['mobile_admin_id']);
        while ($rows = $db->FetchRow($result))
        {
            $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, sent_time, " .
                        ...
                        "VALUES ('".$_SESSION['mobile_admin_id']."', '".$rows['user_id']."', '" .gmtime(). "', ".
    ...
    else
    {
        ...
        foreach ($rec_arr AS $key => $id)
        {
            $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, ".
                   ...
                   "VALUES ('".$_SESSION['mobile_admin_id']."', '$id', '".gmtime()."', ".
                        "'0', '0', '0', '$_POST[title]', '$_POST[message]')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    $smarty->assign('action_link', array('href' => 'message.php?act=list', 'text' => $_LANG['msg_list']));
    $smarty->assign('admin_user',  $_SESSION['mobile_admin_name']);
    $smarty->assign('msg_arr',     $msg_arr);
...
elseif ($_REQUEST['act'] == 're_msg')
{
    $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, sent_time, ".
                "read_time, readed, deleted, title, message) ".
           "VALUES ('".$_SESSION['mobile_admin_id']."', '$_POST[receiver_id]', '" . gmtime() . "', ".
                "0, '0', '0', '$_POST[title]', '$_POST[message]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'drop_msg')
{
    if (isset($_POST['checkboxes']))
    {
        ...
        foreach ($_POST['checkboxes'] AS $key => $id)
        {
            $sql = "UPDATE " .$ecs->table('admin_message'). " SET ".
                   "deleted = '1'".
                   "WHERE message_id = '$id' AND (sender_id='$_SESSION[mobile_admin_id]' OR receiver_id='$_SESSION[mobile_admin_id]')";
            $db->query($sql);
...
function get_message_list()
{
    ...
    {
        case 1:
            $where = " a.receiver_id='" .$_SESSION['mobile_admin_id']. "'";
            break;
        case 2:
           $where = " a.sender_id='".$_SESSION['mobile_admin_id']."' AND a.deleted='0'";
           break;
        case 3:
           $where = " a.readed='0' AND a.receiver_id='".$_SESSION['mobile_admin_id']."' AND a.deleted='0'";
           break;
        case 4:
           $where = " a.readed='1' AND a.receiver_id='".$_SESSION['mobile_admin_id']."' AND a.deleted='0'";
           break;
        default:
           $where = " (a.receiver_id='".$_SESSION['mobile_admin_id']."' OR a.sender_id='" .$_SESSION['mobile_admin_id']. "') AND a.deleted='0'";
    }
