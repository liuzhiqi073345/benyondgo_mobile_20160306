...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if ($row)
    {
        ...
        $_SESSION['mobile_suppliers_id'] = $row['suppliers_id'];
		if(empty($row['ec_salt']))
	    {
			...
			$new_possword=md5(md5($_POST['password']).$ec_salt);
             $db->query("UPDATE " .$ecs->table('admin_user').
                 ...
                 " WHERE user_id='$_SESSION[mobile_admin_id]'");
		}
...
{
    /* 不能编辑demo这个管理员 */
    if ($_SESSION['mobile_admin_name'] == 'demo')
    {
    ...
    /* 查看是否有权限编辑其他管理员的信息 */
    if ($_SESSION['mobile_admin_id'] != $_REQUEST['id'])
    {
        admin_priv('admin_manage');
    }
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    else
    {
        ...
        $admin_id = $_SESSION['mobile_admin_id'];
   ...
   {
       $sess->delete_spec_admin_session($_SESSION['mobile_admin_id']);
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$ecs->table('admin_user'). " WHERE user_id = '".$_SESSION['mobile_admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    if ($_SESSION['mobile_admin_id'] == $_GET['id'])
    {
        admin_priv('all');
    }
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    $db->query($sql);
    /* 动态更新管理员的SESSION */
    if ($_SESSION["mobile_admin_id"] == $_POST['id'])
    {
        $_SESSION["mobile_action_list"] = $act_list;
    }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 管理员不能删除自己 */
    if ($id == $_SESSION['mobile_admin_id'])
    {
