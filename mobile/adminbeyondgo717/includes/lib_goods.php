...
function handle_link_goods($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[mobile_admin_id]'";
    $GLOBALS['db']->query($sql);
...
function get_linked_goods($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND lg.admin_id = '$_SESSION[mobile_admin_id]'";
    }
    $row = $GLOBALS['db']->getAll($sql);
...
function get_group_goods($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND gg.admin_id = '$_SESSION[mobile_admin_id]'";
    }
    $row = $GLOBALS['db']->getAll($sql);
...
function get_goods_articles($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND g.admin_id = '$_SESSION[mobile_admin_id]'";
    }
    $row = $GLOBALS['db']->getAll($sql);
