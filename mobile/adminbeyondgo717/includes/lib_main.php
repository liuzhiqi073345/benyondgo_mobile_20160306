...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
            " VALUES ('" . gmtime() . "', $_SESSION[mobile_admin_id], '" . stripslashes($log_info) . "', '" . real_ip() . "')";
...
{
    $_SESSION['mobile_admin_id']    = $user_id;
    $_SESSION['mobile_admin_name']  = $username;
    $_SESSION['mobile_action_list'] = $action_list;
    $_SESSION['mobile_last_check']  = $last_time; // 用于保存最后一次检查订单的时间
}
...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    if ($_SESSION['mobile_action_list'] == 'all')
    {
        return true;
    }

    if (strpos(',' . $_SESSION['mobile_action_list'] . ',', ',' . $priv_str . ',') === false)
    {
...
{
    return (preg_match('/,*'.$authz.',*/', $_SESSION['mobile_action_list']) || $_SESSION['mobile_action_list'] == 'all');
}
